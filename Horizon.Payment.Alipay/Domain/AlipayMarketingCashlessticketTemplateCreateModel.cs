...
{
    /// <summary>
    /// AlipayMarketingCashlessticketTemplateCreateModel Data Structure.
    /// </summary>
    public class AlipayMarketingCashlessticketTemplateCreateModel : AlipayObject
    {
        /// <summary>
        ...
        /// <summary>
        /// 备注字段，商家可用于填写一些额外的说明，不能超过个128字
        /// </summary>
        [JsonPropertyName("memo")]
        public string Memo { get; set; }

        /// <summary>
        /// 票变动异步通知地址，传入此字段后，票的核销将会进行异步通知，通知具体内容见下方触发异步通知字段描述
        /// </summary>
        ...
        /// <summary>
        /// 规则配置，JSON字符串，{"PID": "2088512417841101,2088512417841102", "STORE": "123456,678901"}，其中PID表示可以核销该票的pid列表，多个值用英文逗号隔开，PID为必传且需与接口调用PID同属一个商家，STORE表示可以核销该票的内部门店ID，非必填。PID和STOREID的最大数量均不能超过3000，允许用户使用MerchantMarketingSign字段自定义核销规则。
        /// </summary>
        ...
        /// <summary>
        /// 票使用说明。JSON数组字符串，最多可以有10条，每条最多50字。如果未传入该字段，将填充为默认描述
        /// </summary>
        [JsonPropertyName("ticket_description")]
        public string TicketDescription { get; set; }

        /// <summary>
        /// 拟发行票的数量。此值在无资金定额票（CASHLESS_FIX_TICKET）下必填，无资金不定额票（CASHLESS_RANDOM_TICKET）下不能填写。单位为张。该数值必须是大于0的整数，最大票发行数量不能超过10,000,000
        /// </summary>
        [JsonPropertyName("ticket_quantity")]
        public long TicketQuantity { get; set; }

        /// <summary>
        /// 票类型，取值范围为：  1. 定额无资金代金票：CASHLESS_FIX_TICKET，在创建时需要传入票面额； 2. 不定额无资金代金票 CASHLESS_RANDOM_TICKET，在票发放时传入票面额；
        /// </summary>
        ...
        /// <summary>
        /// 票可发放总金额（仅用于无资金不定额票，无资金定额票不能填写此值）。币种为人民币，单位为元。该数值需大于等于1，小于等于1,000,000,000，小数点以后最多保留两位。
        /// </summary>
